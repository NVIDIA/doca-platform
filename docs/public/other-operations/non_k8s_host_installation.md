...
export DOCA_URL="https://linux.mellanox.com/public/repo/doca/2.10.0/ubuntu24.04/x86_64/"
...
EOF
  ...
  local attempt=0
  while [ -z "$secret_name" ] && [ $attempt -lt $max_attempts ]; do
    secret_name=$($kubectl_cmd get cert -n $namespace $dpu_node_name-dms-server-cert -o jsonpath='{.spec.secretName}')
    if [ -z "$secret_name" ]; then
      log "Waiting for certificate secret to be created... (attempt $((attempt+1))/$max_attempts)"
      sleep 2
      ((attempt++))
    fi
  done
  ...
  if [ -z "$secret_name" ]; then
    error "Timed out waiting for certificate secret to be created"
  fi
  log "Certificate secret name: $secret_name"
  
  # pull certificate files from secret
  mkdir -p $dms_conf_dir/certs
  $kubectl_cmd get secret -n $namespace $secret_name -o jsonpath='{.data.ca\.crt}' | base64 --decode > $dms_conf_dir/certs/ca.crt
  $kubectl_cmd get secret -n $namespace $secret_name -o jsonpath='{.data.tls\.crt}' | base64 --decode > $dms_conf_dir/certs/tls.crt
  $kubectl_cmd get secret -n $namespace $secret_name -o jsonpath='{.data.tls\.key}' | base64 --decode > $dms_conf_dir/certs/tls.key
}
...
create_dpu_device() {
  ...
  local name="$dpu_node_name-$pci_addr"
...
main() {
  ...
  if [ "$cmd" = "$CMD_REGISTER" ]; then
    ...
    # Create and enable restore-vf service only in non-Kubernetes environment
    if [ "$k8s_env" != true ]; then
        create_restore_vf_service
    else
        log "Skipping restore-vf service creation in Kubernetes environment"
    fi
    ...
    local pci_addr=$(get_the_first_pci_address "$pci_addr_file")
    log "Get PCI address: $pci_addr"
    readlink /sys/bus/pci/devices/"$pci_addr".[01] /sys/bus/pci/devices/"$pci_addr".[01]/virtfn* | xargs -n1 basename | sort -u | while read pci_device; do devlink health set pci/$pci_device reporter fw_fatal grace_period 0; devlink health recover pci/$pci_device reporter fw_fatal; done
