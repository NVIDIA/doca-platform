...
spec:
  ...
  helmChart:
    source:
      ...
      version: 1.1.5
    values:
      containerImage: nvcr.io/nvidia/doca/doca_firefly:1.7.1-doca3.0.0
      hostNetwork: false
...
spec:
  ...
  helmChart:
    source:
      ...
      version: 1.1.5
    values:
      containerImage: nvcr.io/nvidia/doca/doca_firefly:1.7.1-doca3.0.0-host
      hostNetwork: false
...
Defines the trusted [Scalable Function (SF)](https://docs.nvidia.com/networking/display/bluefielddpuosv385/scalable+functions) for the Firefly service.
...
For information about OVN Kubernetes configuration see the [OVN-only user guide](../user-guides/ovn_only/README.md)
...
to the SFC bridge using a `DPUServiceNAD` (Network Attachment Definition). The `DPUService` controller takes care of injecting the correct
resource to the Pod using that [`DPUServiceNAD.yaml`](#DPUServiceNAD).
