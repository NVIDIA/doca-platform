...
config:
  receivers:
    ...
    # TLS OTLP receiver for DPU clusters that verify this collector's serving
    # certificate (DPFOperatorConfig spec.monitoring.openTelemetryCollector.logging.caSecretRef).
    # The certificate is created by hooks/apply-otel-certificates.sh.
    otlp/tls:
      protocols:
        grpc:
  ...
  service:
    pipelines:
      logs:
        receivers: [otlp, otlp/tls, filelog]
...
ports:
  ...
  otlp-tls:
    ...
    containerPort: 4319
    servicePort: 4319
    # Fixed NodePort for the TLS OTLP/gRPC receiver.
    nodePort: 30052
    protocol: TCP
  otlp-https:
    enabled: true
    containerPort: 4320
    servicePort: 4320
    # Fixed NodePort for the TLS OTLP/HTTP receiver.
    nodePort: 30051
    protocol: TCP
  metrics:
    enabled: true
...
# Serving certificate for the TLS OTLP receivers, created by
# hooks/apply-otel-certificates.sh.
extraVolumes:
