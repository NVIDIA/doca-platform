...
The system components and images for DPF are published to publicly available repos that have no authentication. Users
who consume these artifacts from registries with authentication and will need to create Kubernetes Secrets to manage access.
...
```bash
envsubst < using-private-registries-argocd-repository-secret.yaml | kubectl apply -f -
```

[embedmd]:#(using-private-registries-argocd-repository-secret.yaml)
```yaml
apiVersion: v1
kind: Secret
metadata:
