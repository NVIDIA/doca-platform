...
When your worker hosts are not managed by Kubernetes, you will need to perform a manual reboot step during the DPU
provisioning process:

1. When the DPU reaches the `rebooting` phase, you will need to manually power-cycle the worker host.

2. After the worker node has rebooted, remove the `provisioning.dpu.nvidia.com/dpunode-external-reboot-required` annotation
