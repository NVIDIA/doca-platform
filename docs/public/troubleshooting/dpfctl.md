...
You can also download it directly from the command line (in this example we are using `dpfctl-linux-amd64` with version `v25.10.0`):

```shell
curl -L -o /usr/local/bin/dpfctl https://api.ngc.nvidia.com/v2/resources/nvidia/doca/dpfctl/versions/v25.10.0/files/dpfctl-linux-amd64
