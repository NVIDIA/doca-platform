...
generate-client-for-storage-nvidia-external-attacher: client-gen lister-gen informer-gen deepcopy-gen # Generate client/lister/informer for sv-volumeattachment
	...
	--input-base $(CURDIR) --input third_party/forked/nvidia-external-attacher/api/storage/v1alpha1 \
	...
	$(LISTER_GEN) $(CURDIR)/third_party/forked/nvidia-external-attacher/api/storage/v1alpha1 \
	...
	$(INFORMER_GEN) $(CURDIR)/third_party/forked/nvidia-external-attacher/api/storage/v1alpha1 \
...
test-deploy-operator-helm: helm helm-package-operator ## Deploy the DPF Operator using helm
	...
	$(MAKE) HELMFILE_FILE=$(CURDIR)/deploy/helmfiles/monitoring.yaml test-deploy-helmfile
...
test-deploy-helmfile: helmfile helm helm-diff ## Deploy helm dependencies using helmfile
	$(HELMFILE) apply \
	  --suppress-diff \
...
E2E_TEST_ARGS ?= -v -ginkgo.v -ginkgo.fail-fast -ginkgo.label-filter="DPFSystem && !SDN" -e2e.config=./config-quick.yaml
...
binary-storage-nvidia-external-attacher: generate-client-for-storage-nvidia-external-attacher ## Build the nvidia external attacher binary.
	...
	./$(NVIDIA_EXTERNAL_ATTACHER_DIR)/hack/client.sh $(CURDIR) $(EXTERNAL_ATTACHER_BRANCH)
