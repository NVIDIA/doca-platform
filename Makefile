...
VPC_REF=8f788dfc81eb088950d83d238f9b04e89eec6e17
...
test-release-e2e-slow: export RELEASE_MANIFEST_ENABLED := true
...
# Set to true by release (and test-helper push targets) to record each pushed image/chart.
...
VERIFY_DPU_NETWORKING_MANIFESTS ?= flannel multus sriov-device-plugin nvidia-k8s-ipam ovs-cni servicechainset-controller sfc-controller cni-installer node-problem-detector kube-state-metrics dpu-monitoring opentelemetry-collector
...
docker-push-ovs-cni: ## Push the docker image for ovs-cni
	...
	@$(call record_release_artifact,image,$(OVS_CNI_IMAGE_NAME),$(OVS_CNI_IMAGE):$(TAG),release)
