...
func ProvisionDPUCluster(ctx context.Context, input ProvisionDPUClustersInput) {
	...
	Expect(input.client.Create(ctx, input.dpuCluster)).To(Succeed())
	...
	Eventually(func(g Gomega) {
		...
		g.Expect(input.client.Create(ctx, bfb)).To(Succeed())
	...
	Eventually(func(g Gomega) {
		...
		g.Expect(input.client.Create(ctx, dpuFlavor)).To(Succeed())
	}).WithTimeout(60 * time.Second).Should(Succeed())
}
...
func ProvisionDPUSet(ctx context.Context, input ProvisionDPUClustersInput) {
	Eventually(func(g Gomega) {
		...
		g.Expect(input.client.Create(ctx, dpuset)).To(Succeed())
	}).WithTimeout(60 * time.Second).Should(Succeed())
...
func getDPUClusterClient(ctx context.Context, input ProvisionDPUClustersInput) {
	Eventually(func(g Gomega) {
		var err error
		dpuClusters, err := dpucluster.GetConfigs(ctx, input.client)
...
func collectResourcesAndLogs(ctx context.Context, input collectResourcesInput, dpuClusterClientOptions []dpucluster.ClientOption, testName string) error {
	...
	cc := collector.ClusterCollector{
		...
		ClientOptions: dpuClusterClientOptions,
	}
