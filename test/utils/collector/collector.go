...
type ClusterCollector struct {
	Client        client.Client
	ClientSet     *kubernetes.Clientset
	ClientOptions []dpucluster.ClientOption
}
...
func GetClusterCollectors(ctx context.Context, cc ClusterCollector, artifactsDirectory string, inventoryManifestsDirectory string) ([]*Cluster, error) {
	...
	for _, conf := range clusterConfigs {
		dpuClusterClient, err := conf.Client(ctx, cc.ClientOptions...)
		if err != nil {
			errs = append(errs, err)
			continue
		}
		dpuClusterClientset, _, err := conf.Clientset(ctx, cc.ClientOptions...)
		if err != nil {
			errs = append(errs, err)
			continue
		}
