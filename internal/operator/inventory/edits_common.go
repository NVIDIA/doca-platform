	...
	return func(obj client.Object) error {
		...
		for i := range daemonset.Spec.Template.Spec.Containers {
			if daemonset.Spec.Template.Spec.Containers[i].Name == containerName {
				daemonset.Spec.Template.Spec.Containers[i].Resources = resources
				return nil
			}
		}
...
func EnvForDaemonSetContainerEdit(containerName string, envs []corev1.EnvVar) StructuredEdit {
	return func(obj client.Object) error {
		...
		// Create a map to find the container by name
		containerIndex := -1
		for i, container := range ds.Spec.Template.Spec.Containers {
			if container.Name == containerName {
				containerIndex = i
		...
		container := ds.Spec.Template.Spec.Containers[containerIndex]

		// Create a map of existing env vars by name for easy lookup
		existingEnvs := make(map[string]int)
		for j, env := range container.Env {
			existingEnvs[env.Name] = j
		}

		// Process each env var in the input list
		for _, env := range envs {
			if idx, exists := existingEnvs[env.Name]; exists {
		...
		ds.Spec.Template.Spec.Containers[containerIndex] = container
		return nil
	}
}
