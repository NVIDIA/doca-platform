...
// IsReady returns an error if the DPUService does not have a Ready status condition and the version is not updated.
func (f *fromDPUService) IsReady(ctx context.Context, c client.Client, namespace string) error {
	return f.isReady(ctx, c, namespace, false)
}

// IsReadyAndVersionUpdated returns an error if the DPUService does not have a Ready status condition.
func (f *fromDPUService) IsReadyAndVersionUpdated(ctx context.Context, c client.Client, namespace string) error {
