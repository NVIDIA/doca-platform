...
func (p *provisioningControllerObjects) IsReady(ctx context.Context, c client.Client, namespace string) error {
	return deploymentReadyCheck(ctx, c, namespace, p.objects, false)
}

// IsReadyAndVersionUpdated reports the readiness of the provisioning controller objects as well as the version state.
// It returns an error when the number of Replicas in the single provisioning controller deployment is true.
func (p *provisioningControllerObjects) IsReadyAndVersionUpdated(ctx context.Context, c client.Client, namespace string) error {
