	...
	t.Run("simulate simple update and verify it passes", func(t *testing.T) {
		...
		config := &operatorv1.DPFOperatorConfig{
			...
			Spec: operatorv1.DPFOperatorConfigSpec{
				ProvisioningController: operatorv1.ProvisioningControllerConfiguration{
					BFBPersistentVolumeClaimName: "test-pvc",
					Disable:                      ptr.To(true), // Disable provisioning controller for this test
				},
				// Enable one system component to test dynamic validation
				ServiceSetController: &operatorv1.ServiceSetControllerConfiguration{
					Disable: ptr.To(false), // Enable this component
				},
				// Disable other system components to avoid complexity
				Multus:               &operatorv1.MultusConfiguration{Disable: ptr.To(true)},
				SRIOVDevicePlugin:    &operatorv1.SRIOVDevicePluginConfiguration{Disable: ptr.To(true)},
				Flannel:              &operatorv1.FlannelConfiguration{Disable: ptr.To(true)},
				OVSCNI:               &operatorv1.OVSCNIConfiguration{Disable: ptr.To(true)},
				NVIPAM:               &operatorv1.NVIPAMConfiguration{Disable: ptr.To(true)},
				SFCController:        &operatorv1.SFCControllerConfiguration{Disable: ptr.To(true)},
				DPUServiceController: &operatorv1.DPUServiceControllerConfiguration{Disable: ptr.To(true)},
				DPUDetector:          &operatorv1.DPUDetectorConfiguration{Disable: ptr.To(true)},
				KamajiClusterManager: &operatorv1.KamajiClusterManagerConfiguration{Disable: ptr.To(true)},
				StaticClusterManager: &operatorv1.StaticClusterManagerConfiguration{Disable: ptr.To(true)},
			},
		}
		g.Expect(testClient.Create(ctx, config)).To(Succeed())
