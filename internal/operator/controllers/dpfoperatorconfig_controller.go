...
// +kubebuilder:rbac:groups=provisioning.dpu.nvidia.com,resources=dpusets;dpuflavors;bfbs;dpus;dpuclusters;dpudevices;dpunodes;dpudiscoveries,verbs=create;delete;get;list;watch;patch;update;deletecollection
...
func (r *DPFOperatorConfigReconciler) updateSystemComponentStatus(ctx context.Context, config *operatorv1.DPFOperatorConfig, dpuClusters []*dpucluster.Config) {
	...
	for _, component := range r.Inventory.EnabledComponents(inventory.VariablesFromDPFOperatorConfig(r.Defaults, config, dpuClusters)) {
		err := component.IsReadyAndVersionUpdated(ctx, r.Client, config.Namespace)
		if err != nil {
	...
	if len(errs) > 0 {
		message := fmt.Sprintf("System components must be ready for DPF Operator to continue:\n%v",
