...
// +kubebuilder:rbac:groups=provisioning.dpu.nvidia.com,resources=dpusets/status;dpus/status;bfbs/status;dpuclusters/status;dpunodes/status;dpudevices/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=provisioning.dpu.nvidia.com,resources=dpusets/finalizers;bfbs/finalizers;dpus/finalizers;dpuflavors/finalizers;dpuclusters/finalizers;dpudevices/finalizers;dpunodes/finalizers,verbs=update
...
func (r *DPFOperatorConfigReconciler) updateSystemComponentStatus(ctx context.Context, config *operatorv1.DPFOperatorConfig, dpuClusters []*dpucluster.Config) {
	...
	if len(unreadyMessages) > 0 {
		message := conditions.ReadyConditionMessage("System components are not ready", unreadyMessages)
		conditions.AddFalse(
			config,
