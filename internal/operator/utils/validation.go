...
var allowSourceVersions = []string{"v25.7.0"}
...
func isSameMajorMinor(prevVersion *semver.Version) bool {
	// Create a tilde constraint for the target version to allow patch-level changes within same major.minor.
	constraint, _ := semver.NewConstraint(fmt.Sprintf("~%s", release.DPFVersion()))
...
func isAllowedSourceVersion(prevVersion *semver.Version, allowedSources []string) bool {
	for _, allowed := range allowedSources {
		...
		tilde, _ := semver.NewConstraint(fmt.Sprintf("~%s-0", allowed))
		if tilde != nil && tilde.Check(prevVersion) {
			return true
		}
	}
	return false
...
func ValidateDPFVersion(prevVersionStr *string) error {
	...
	if isSameMajorMinor(prevVersion) {
		return nil
	}
