...
const (
	...
	// TODO: these annotations do not align with the new flow and should be removed. Here's how to simulate the new flow with mock dms:
	//   1. As we no longer retrieve the address of DMS server from the DMS pod,
	//      the mock dms should set its IP and listener port in DPUNode.spec.DMSAddress
	//   2. No host network pod will be created in the new flow.
	// 		Instead, the provisioning controller calls DMS debug API to run the hostnetwork script on the host.
	//      As a result, the annotaion can be removed, and the Get service of the mock dms should be extended
...
func GenerateDMSPodName(dpuNodeName string) string {
	return fmt.Sprintf("%s-%s", dpuNodeName, "dms")
}
