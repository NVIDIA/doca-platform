...
func (r *DPUSetReconciler) createDPU(ctx context.Context, dpuSet *provisioningv1.DPUSet, dpuDevice *provisioningv1.DPUDevice) error {
	...
	labels := map[string]string{
		...
		cutil.DPUNodeNameLabel:     dpuDevice.Labels[cutil.DPUNodeNameLabel],
	}
	...
	dpu := &provisioningv1.DPU{
		...
		Spec: provisioningv1.DPUSpec{
			DPUNodeName:   dpuDevice.Labels[cutil.DPUNodeNameLabel],
			...
			PCIAddress: dpuDevice.Spec.PCIAddress,
			BMCIP:      bmcIP,
		},
	}
...
func (r *DPUSetReconciler) createMissingDPUs(ctx context.Context, dpuSet *provisioningv1.DPUSet, dpuDeviceMap map[string]provisioningv1.DPUDevice, dpuMap map[string]provisioningv1.DPU) error {
	...
	for dpuDeviceName, dpuDevice := range dpuDeviceMap {
		if _, exists := dpuMap[dpuDeviceName]; exists {
			logger.V(2).Info(fmt.Sprintf("DPU skipped: (%s/%s) for DPUDevice: %s", dpuSet.Namespace, dpuSet.Name, dpuDeviceName))
			continue
		}
		if err := r.createDPU(ctx, dpuSet, &dpuDevice); err != nil {
			return err
		}
	}
	return nil
