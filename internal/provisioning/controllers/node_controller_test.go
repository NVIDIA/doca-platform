...
var _ = Describe("Node Controller", func() {
	...
	Describe("Node", func() {
			...
			It("Node: create a DPUNode before creating Node - validate No DMS Pod is created", func() {
				...
				Consistently(func(g Gomega) {
					...
					g.Expect(k8sClient.Get(ctx, client.ObjectKey{
						Namespace: operatorcontroller.DefaultDPFOperatorConfigSingletonNamespace,
						Name:      cutil.GenerateDMSPodName(node.Name)},
		...
		AfterEach(func() {
			By("deleting the Pod")
			Expect(client.IgnoreNotFound(k8sClient.Delete(ctx, &corev1.Pod{ObjectMeta: metav1.ObjectMeta{Namespace: operatorcontroller.DefaultDPFOperatorConfigSingletonNamespace, Name: cutil.GenerateDMSPodName("dpf-provisioning-node-controller-test-2")}}))).To(Succeed())
	...
	var _ = Describe("DMS Pod", func() {
		...
		Context("obj test context", func() {
			...
			It("DMS Pod: creating DMS Pod with env", func() {
				...
				By("checking Pod env")
				pod := &corev1.Pod{}
				Expect(k8sClient.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: cutil.GenerateDMSPodName(testNode.Name)}, pod)).To(Succeed())
