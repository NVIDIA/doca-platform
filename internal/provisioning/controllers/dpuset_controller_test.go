...
var _ = Describe("DPUSet", func() {
	...
	var createDPUDevice = func(ctx context.Context, namespace string, name string, pciAddress string) *provisioningv1.DPUDevice {
	...
	BeforeEach(func() {
		...
		testDPUDevice = createDPUDevice(ctx, testNS.Name, dpuDeviceName, DefaultPCIAddress)
	...
	Context("obj test context", func() {
		...
		It("DPUSet: should create few DPUs", func() {
			...
			dpuDevice5 := createDPUDevice(ctx, testNS.Name, "dpu-device5", "0000-55-00")
			DeferCleanup(func() {
				Expect(client.IgnoreNotFound(k8sClient.Delete(ctx, dpuDevice5))).To(Succeed())
			})
			dpuDevice6 := createDPUDevice(ctx, testNS.Name, "dpu-device6", "0000-66-00")
			DeferCleanup(func() {
		...
		It("DPUSet: should be removed in case the DPUDevice disappeared", func() {
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(dpuList.Items[0].Name).To(Equal(testDPUDevice.Name))
			}).WithTimeout(10 * time.Second).Should(Succeed())
