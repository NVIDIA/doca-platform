...
func InitializeInterface(ctx context.Context, dpu *provisioningv1.DPU, ctrlCtx *dutil.ControllerContext) (provisioningv1.DPUStatus, error) {
	...
	basicAuthClient, err := rfclient.InitPassword(ctx, *device.Spec.BMCIP, dpu.Namespace, ctrlCtx.Client)
	if err != nil {
	...
	_, err = rfclient.NewTLSClient(ctx, device, ctrlCtx.Client)
	if err != nil {
		log.FromContext(ctx).V(3).Info("failed to create tls client", err)

		if err = setUpMTLS(ctx, dpu, device, ctrlCtx, basicAuthClient); err != nil {
...
func setUpMTLS(ctx context.Context, dpu *provisioningv1.DPU, device *provisioningv1.DPUDevice, ctrlCtx *dutil.ControllerContext, basicAuthClient *rfclient.Client) error {
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			...
			resp, csrInfo, err := basicAuthClient.GenerateCSR(*device.Spec.BMCIP)
			if err != nil {
...
func checkCapacity(ctx context.Context, dpu *provisioningv1.DPU, device *provisioningv1.DPUDevice, ctrlCtx *dutil.ControllerContext) (dutil.CapacityResult, error) {
	...
	tlsClient, err := rfclient.NewTLSClient(ctx, device, ctrlCtx.Client)
	if err != nil {
		return dutil.CapacityUnknown, err
	}
