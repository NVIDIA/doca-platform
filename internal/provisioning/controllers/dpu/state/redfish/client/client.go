...
func NewRawClient(BMCIP string) (*Client, error) {
	if net.ParseIP(BMCIP) == nil {
		return nil, fmt.Errorf("invalid IP: %s", BMCIP)
	}
	return &Client{Client: resty.New().SetBaseURL(fmt.Sprintf("https://%s", BMCIP)).SetTLSClientConfig(&tls.Config{InsecureSkipVerify: true})}, nil
}
...
func InitPassword(ctx context.Context, BMCIP string, namespace string, k8sClient client.Client) (*Client, error) {
	if net.ParseIP(BMCIP) == nil {
		return nil, fmt.Errorf("invalid IP: %s", BMCIP)
	}
	...
	client, err := NewBasicAuthClient(BMCIP, BMCUser, passwd)
	if err != nil {
		return nil, err
	}
	...
	case http.StatusUnauthorized:
		...
		defaultClient, err := NewBasicAuthClient(BMCIP, BMCUser, BMCDefaultPassword)
		if err != nil {
			return nil, err
		}
