...
func ConfigFWParameters(ctx context.Context, dpu *provisioningv1.DPU, ctrlCtx *dutil.ControllerContext) (provisioningv1.DPUStatus, error) {
	...
	if err != nil {
		...
		cutil.SetDPUCondition(state, cutil.NewCondition(string(provisioningv1.DPUConfigFWParameters), err, "FailedToCheckBMCFW", err.Error()))
		return *state, err
	}
	log.FromContext(ctx).Info(fmt.Sprintf("BMC FW version %q", data.Version))
	if !validateBMCFW(data.Version, "") {
		err = fmt.Errorf("old BMC firmware: %q", data.Version)
		cutil.SetDPUCondition(state, cutil.NewCondition(string(provisioningv1.DPUConfigFWParameters), err, "OldBMCFW", err.Error()))
	...
	if err != nil {
		err = fmt.Errorf("failed to get DPU NIC: %w", err)
		cutil.SetDPUCondition(state, cutil.NewCondition(string(provisioningv1.DPUConfigFWParameters), err, "FailedToGetCheckDPUNIC", err.Error()))
		return *state, err
	}
	log.FromContext(ctx).Info(fmt.Sprintf("DPU NIC version %q", data.Version))
	if !validateDPUNIC(data.Version, "") {
		err = fmt.Errorf("old DPU NIC: %q", data.Version)
		cutil.SetDPUCondition(state, cutil.NewCondition(string(provisioningv1.DPUConfigFWParameters), err, "OldDPUNIC", err.Error()))
		state.Phase = provisioningv1.DPUError
		return *state, nil
	}
	...
	if err != nil {
		err = fmt.Errorf("failed to get DPU OS: %w", err)
		cutil.SetDPUCondition(state, cutil.NewCondition(provisioningv1.DPUCondFWConfigured.String(), err, "FailedToGetCheckDPUOS", err.Error()))
		return *state, err
	}
	logger.Info(fmt.Sprintf("DPU OS version %q", data.Version))
	if !validateDPUOS(data.Version, "") {
		err = fmt.Errorf("old DPU OS: %q", data.Version)
		state.Phase = provisioningv1.DPUError
		cutil.SetDPUCondition(state, cutil.NewCondition(string(provisioningv1.DPUConfigFWParameters), err, "OldDPUOS", err.Error()))
		return *state, nil
	}
