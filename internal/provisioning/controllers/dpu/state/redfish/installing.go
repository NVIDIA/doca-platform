...
func Installing(ctx context.Context, dpu *provisioningv1.DPU, ctrlCtx *dutil.ControllerContext) (provisioningv1.DPUStatus, error) {
	...
	if cond == nil {
		...
		// TODO: task 100% progress does not mean the os installation is finished,
		// it just means the BFB transfer is finished.
		// wait 10 minutes to make sure the os installation and cloud-init are finished in this version.
		if time.Since(cond.LastTransitionTime.Time) < 10*time.Minute {
			return *state, nil
		}
	}
