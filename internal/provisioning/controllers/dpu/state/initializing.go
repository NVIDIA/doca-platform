...
func Initializing(ctx context.Context, dpu *provisioningv1.DPU, ctrlCtx *dutil.ControllerContext) (provisioningv1.DPUStatus, error) {
	...
	// Add finalizer to the DPUNode object to ensure that it's not deleted before the DPU is gone
	dpuNodeNeedsUpdate := controllerutil.AddFinalizer(dpuNode, provisioningv1.DPUNodeFinalizer)
	if dpuNodeNeedsUpdate {
		if err := ctrlCtx.Update(ctx, dpuNode); err != nil {
			err = fmt.Errorf("failed to update DPUNode: %w", err)
			cutil.SetDPUCondition(state, cutil.NewCondition(provisioningv1.DPUCondInitialized.String(), err, "UpdateDPUNodeError", err.Error()))
			return *state, err
		}
	}
	...
	if _, ok := dpuNode.GetLabels()[cutil.NodeFeatureDiscoveryLabelPrefix+cutil.DPUOOBBridgeConfiguredLabel]; !ok {
