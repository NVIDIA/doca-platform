...
func ConfigFWParameters(ctx context.Context, dpu *provisioningv1.DPU, ctrlCtx *dutil.ControllerContext) (provisioningv1.DPUStatus, error) {
	...
	if resp, err := gnmiClient.Get(ctx, getModeRequest); err == nil {
		for _, notification := range resp.Notification {
			for _, update := range notification.Update {
				if val := update.Val.GetStringVal(); val != "" {
					currentDPUMode = val
					logger.V(3).Info(fmt.Sprintf("The mode of DPU %s is: %s", dpu.Name, currentDPUMode))
					break
				}
			}
		}
	} else {
		err = fmt.Errorf("failed to get DPU mode: %w", err)
		cutil.SetDPUCondition(state, cutil.NewCondition(provisioningv1.DPUCondFWConfigured.String(), err, "FailedToGetDPUMode", err.Error()))
		return *state, err
	}

	// If the mode of DPU is not DPU mode, we should set the mode to DPU mode.
	if !strings.EqualFold(currentDPUMode, string(flavor.Spec.DpuMode)) {
		...
		if resp, err := gnmiClient.Set(ctx, setModeRequest); err == nil {
			...
			cutil.SetDPUCondition(state, cutil.NewCondition(provisioningv1.DPUCondFWConfigured.String(), nil, string(provisioningv1.DPUCondReasonModeUpdate), ""))
...
func getModeRequest() *gnmi.GetRequest {
