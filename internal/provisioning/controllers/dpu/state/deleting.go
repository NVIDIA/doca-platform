...
func Deleting(ctx context.Context, dpu *provisioningv1.DPU, ctrlCtx *dutil.ControllerContext) (provisioningv1.DPUStatus, error) {
	...
	if err := RemoveNodeEffect(ctx, ctrlCtx.Client, dpu); err != nil {
		err = fmt.Errorf("failed to remove node effect: %w", err)
		cutil.SetDPUCondition(state, cutil.NewCondition(provisioningv1.DPUCondDeleting.String(), err, "RemoveNodeEffectError", err.Error()))
		return *state, err
	}
	...
	// TODO: This is not safe for multi DPU per DPUNode, implementation needs adjustment
	if err := removeFinalizerFromDPUNode(ctx, ctrlCtx.Client, dpu); err != nil {
		err = fmt.Errorf("failed to remove finalizer from the DPUNode object: %w", err)
		cutil.SetDPUCondition(state, cutil.NewCondition(provisioningv1.DPUCondDeleting.String(), err, "RemoveDPUNodeFinalizerError", err.Error()))
		return *state, err
	}
...
// removeFinalizerFromDPUNode removes the finalizer from the DPUNode object to ensure that it can be deleted
func removeFinalizerFromDPUNode(ctx context.Context, client crclient.Client, dpu *provisioningv1.DPU) error {
	dpuNode := &provisioningv1.DPUNode{}
	if err := client.Get(ctx, types.NamespacedName{Namespace: dpu.Namespace, Name: dpu.Spec.DPUNodeName}, dpuNode); err != nil {
	...
	if err := client.Update(ctx, dpuNode); err != nil {
