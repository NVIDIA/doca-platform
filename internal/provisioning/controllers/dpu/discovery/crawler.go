...
func (c *CrawlerService) Crawl(ctx context.Context, ipRange provisioningv1.IPRange) (int, error) {
	...
	logger.Info("Crawling IP range", "start", ipRange.StartIP, "end", ipRange.EndIP)
...
func (c *CrawlerService) worker(ctx context.Context, wg *sync.WaitGroup, jobs <-chan uint32, results chan<- CrawlResult, existingIPs map[string]bool) {
	...
	for ip := range jobs {
		...
		logger.Info("Processing IP", "ip", ipStr)
		result := CrawlResult{IP: ipStr}

		// Skip if IP already has an associated DPU device
		if existingIPs[ipStr] {
			logger.V(1).Info("Skipping IP - already has existing DPU device", "ip", ipStr)
			result.Error = fmt.Errorf("IP %s already has an associated DPU device", ipStr)
			results <- result
			continue
		}
		...
		logger.Info("Found response from the Redfish BMC at address: %s", ipStr)
