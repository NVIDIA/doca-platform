...
var _ = Describe("DPUNode Controller", func() {
	...
	Describe("DPUNode", func() {
		...
		Context("obj test context", func() {
			...
			It("DPUNode: Create Node and then Delete it - validate DPUNode is deleted", func() {
				By("Create Node and DPUNode")
				node := &corev1.Node{ObjectMeta: metav1.ObjectMeta{
					Name:   "dpunode-test-node-4",
					Labels: map[string]string{"feature.node.kubernetes.io/dpu-enabled": "true"}}}
				Expect(k8sClient.Create(ctx, node)).NotTo(HaveOccurred())

				dpuNode := createDPUNode("dpunode-test-node-4", operatorcontroller.DefaultDPFOperatorConfigSingletonNamespace)
				dpuNode.ObjectMeta.OwnerReferences = append(dpuNode.ObjectMeta.OwnerReferences, metav1.OwnerReference{
					APIVersion: "/v1, Kind=Node",
					Kind:       "Node",
					Name:       node.Name,
				...
				Eventually(func(g Gomega) {
					dpuNodeFetched := &provisioningv1.DPUNode{}
					g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(dpuNode), dpuNodeFetched)).To(Succeed())
					g.Expect(dpuNodeFetched.OwnerReferences).To(ContainElement(metav1.OwnerReference{
						APIVersion: "/v1, Kind=Node",
						Kind:       "Node",
						Name:       node.Name,
						UID:        node.UID,
				...
				By("Delete Node and check DPUNode finalizer is removed")
				Expect(k8sClient.Delete(ctx, node)).To(Succeed())
				Eventually(func(g Gomega) []string {
					dpuNodeFetched := &provisioningv1.DPUNode{}
					g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(dpuNode), dpuNodeFetched)).To(Succeed())
					return dpuNodeFetched.Finalizers
				}).WithTimeout(30 * time.Second).Should(BeEmpty())
			...
			It("DPUNode: Validate DPUDevice params - good flow gNOI", func() {
				...
				Expect(k8sClient.Create(ctx, dpuDevice)).To(Succeed())
