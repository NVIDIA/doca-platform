...
var _ = Describe("DPU", func() {
	...
	var createObj = func(name string) *provisioningv1.DPU {
		return &provisioningv1.DPU{
			...
			Spec:   provisioningv1.DPUSpec{DPUDeviceName: "dpudevice-1"},
	...
	Context("obj test context", func() {
		...
		It("spec.nodeEffect assign nil", func() {
			...
			Expect(k8sClient.Get(ctx, getObjKey(obj), objFetched)).To(Succeed())
			Expect(*objFetched.Spec.NodeEffect).To(Equal(provisioningv1.NodeEffect{Drain: ptr.To(true)}))
		})
		...
		It("spec.pciAddress is immutable", func() {
			...
			obj.Spec.PCIAddress = ptr.To("0000-bb-00")
			err = k8sClient.Update(ctx, obj)
			Expect(err).To(HaveOccurred())
