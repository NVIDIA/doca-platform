...
var _ = Describe("NetworkSelectionElement", func() {
	...
	DescribeTable("Validate AddNetworkAnnotationToServiceDaemonSet",
		...
		Entry("networks is nil", &dpuservicev1.DPUService{}, nil, &dpuservicev1.ServiceDaemonSetValues{
			Annotations: map[string]string{
				networkAnnotationKey: mergeWithInvalidNetwork(""),
			},
		}, nil),
		Entry("networks is empty", &dpuservicev1.DPUService{}, map[string]types.NetworkSelectionElement{}, &dpuservicev1.ServiceDaemonSetValues{
			Annotations: map[string]string{
				networkAnnotationKey: mergeWithInvalidNetwork(""),
			},
		}, nil),
		Entry("DPUService contains annotation", &dpuservicev1.DPUService{
			...
			Annotations: map[string]string{
				networkAnnotationKey: mergeWithInvalidNetwork(`[{"name":"mybrsfc","namespace":"my-namespace","interface":"net1","cni-args":null}]`),
			},
		}, nil),
		Entry("networks contains network for interface", &dpuservicev1.DPUService{
			Spec: dpuservicev1.DPUServiceSpec{
				ServiceDaemonSet: &dpuservicev1.ServiceDaemonSetValues{
					Annotations: map[string]string{
						networkAnnotationKey: mergeWithInvalidNetwork(`[{"name":"mybrsfc","namespace":"my-namespace","interface":"net1"}]`),
					},
				},
			},
		...
		Entry("networks contains network with interface that conflicts, ServiceDaemonSet takes precedence", &dpuservicev1.DPUService{
			Spec: dpuservicev1.DPUServiceSpec{
				ServiceDaemonSet: &dpuservicev1.ServiceDaemonSetValues{
					Annotations: map[string]string{
						networkAnnotationKey: mergeWithInvalidNetwork(`[{"name":"mybrsfc","namespace":"my-namespace","interface":"net1"},{"name":"iprequest","interface":"myip1",` +
		...
		Entry("networks contains network with interface that conflicts, reset cni args", &dpuservicev1.DPUService{
			Spec: dpuservicev1.DPUServiceSpec{
				ServiceDaemonSet: &dpuservicev1.ServiceDaemonSetValues{
					Annotations: map[string]string{
						networkAnnotationKey: mergeWithInvalidNetwork(`[{"name":"mybrsfc","namespace":"my-namespace","interface":"net1","cni-args":{}},` +
...
// mergeWithInvalidNetwork merges the given network annotation with the invalid network
func mergeWithInvalidNetwork(networkAnnotation string) string {
	if networkAnnotation == "" {
		return "[{\"name\":\"invalid-network\",\"namespace\":\"invalid-namespace\",\"interface\":\"invalid-interface\",\"cni-args\":null}]"
	}
	// Remove the opening bracket
	networkAnnotation = networkAnnotation[1:]
	// Add the invalid network and close the array
	return "[{\"name\":\"invalid-network\",\"namespace\":\"invalid-namespace\",\"interface\":\"invalid-interface\",\"cni-args\":null}," + networkAnnotation
}
