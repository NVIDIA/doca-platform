...
var _ = Describe("DPUServiceTemplate Controller", func() {
	...
	Context("When checking the DPUServiceTemplate API validations", func() {
		var testNS *corev1.Namespace
		BeforeEach(func() {
			By("Creating the namespaces")
			testNS = &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{GenerateName: "testns-"}}
		...
		It("should not create the DPUServiceTemplate if spec.deploymentServiceName exceeds the maximum length", func() {
			dpuServiceTemplate := getMinimalDPUServiceTemplate(testNS.Name)
			dpuServiceTemplate.Spec.DeploymentServiceName = utilrand.String(29)
			Expect(testClient.Create(ctx, dpuServiceTemplate)).ToNot(Succeed())
		})
	})
})
