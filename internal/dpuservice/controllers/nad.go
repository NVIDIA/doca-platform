...
import (
	...
	podipam "github.com/nvidia/doca-platform/internal/pod-ipam-injector/controllers"
...
func addNetworkAnnotationToServiceDaemonSet(dpuService *dpuservicev1.DPUService, networkSelectionByInterface map[string]types.NetworkSelectionElement) (*dpuservicev1.ServiceDaemonSetValues, error) {
	...
	// Always add invalid network
	// This network will be removed by the pod-ipam-injector controller
	// once the MTU is injected in the CNI args correctly
	if networkSelectionByInterface == nil {
		networkSelectionByInterface = make(map[string]types.NetworkSelectionElement)
	}
	networkSelectionByInterface[podipam.InvalidNetworkInterface] = generateInvalidNetworkAnnotation()
...
func generateInvalidNetworkAnnotation() types.NetworkSelectionElement {
	return types.NetworkSelectionElement{
		Name:             podipam.InvalidNetworkName,
		Namespace:        podipam.InvalidNetworkNamespace,
		InterfaceRequest: podipam.InvalidNetworkInterface,
	}
}
