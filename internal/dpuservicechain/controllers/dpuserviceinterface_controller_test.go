...
var _ = Describe("DPUServiceInterface Controller", func() {
	Context("When reconciling a resource", func() {
		...
		It("should successfully reconcile a DPUServiceInterface with maximum name length", func() {
			By("Create DPUServiceInterface")
			dsc := getDPUServiceInterface(utilrand.String(63), testNS, &metav1.LabelSelector{})
			Expect(testClient.Create(ctx, dsc)).NotTo(HaveOccurred())
			cleanupObjects = append(cleanupObjects, dsc)
			By("Verify ServiceInterfaceSet is created")
			Eventually(func(g Gomega) {
				scs := &dpuservicev1.ServiceInterfaceSet{ObjectMeta: metav1.ObjectMeta{Name: dsc.Name, Namespace: testNS}}
		...
		It("should fail to create a DPUServiceInterface with name exceeding the maximum length", func() {
			By("Create DPUServiceInterface")
			dsc := getDPUServiceInterface(utilrand.String(64), testNS, &metav1.LabelSelector{})
		...
		It("should successfully reconcile the DPUServiceInterface with virtualNetwork", func() {
			...
			vpc := getDPUVPC("test-vpc", testNS)
			vpc.Spec.IsolationClassName = "test-isolation-class"

			virtualNetwork := getDPUVirtualNetwork("test-virtualnetwork", testNS)
			virtualNetwork.Spec.VPCName = "test-vpc"

			isolationClass := getIsolationClass("test-isolation-class", testNS)
			isolationClass.Spec.Provisioner = "test-provisioner"

			serviceInterface := getDPUServiceInterface(dsiResourceName, testNS, &metav1.LabelSelector{})
			...
			cleanupObjects = append(cleanupObjects, serviceInterface, vpc, virtualNetwork, isolationClass)

			By("Verify ServiceInterfaceSet is created")
			Eventually(func(g Gomega) {
				sis := &dpuservicev1.ServiceInterfaceSet{ObjectMeta: metav1.ObjectMeta{Name: dsiResourceName, Namespace: testNS}}
