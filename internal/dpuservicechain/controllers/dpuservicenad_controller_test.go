/*
Copyright 2025 NVIDIA

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

package controllers

import (
	"encoding/json"
	"time"

	dpuservicev1 "github.com/nvidia/doca-platform/api/dpuservice/v1alpha1"
	provisioningv1 "github.com/nvidia/doca-platform/api/provisioning/v1alpha1"
	"github.com/nvidia/doca-platform/pkg/dpucluster"
	testutils "github.com/nvidia/doca-platform/test/utils"

	"github.com/fluxcd/pkg/runtime/patch"
	. "github.com/onsi/ginkgo/v2"
	. "github.com/onsi/gomega"
	corev1 "k8s.io/api/core/v1"
	apierrors "k8s.io/apimachinery/pkg/api/errors"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	"k8s.io/apimachinery/pkg/apis/meta/v1/unstructured"
	"k8s.io/apimachinery/pkg/runtime/schema"
	utilrand "k8s.io/apimachinery/pkg/util/rand"
	"sigs.k8s.io/controller-runtime/pkg/client"
)

var _ = Describe("DPUServiceNAD Controller", func() {
	Context("When reconciling a resource", func() {
		var testNS *corev1.Namespace
		BeforeEach(func() {
			By("Creating the namespaces")
			testNS = &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{GenerateName: "testns-"}}
			Expect(testClient.Create(ctx, testNS)).To(Succeed())
			DeferCleanup(testClient.Delete, ctx, testNS)
		})
		It("should successfully reconcile the DPUServiceNAD", func() {
			By("Reconciling the created resource")
			dpuServiceNAD := getMinimalDPUServiceNAD(testNS.Name)
			Expect(testClient.Create(ctx, dpuServiceNAD)).To(Succeed())
			DeferCleanup(testutils.CleanupAndWait, ctx, testClient, dpuServiceNAD)
			By("checking that finalizer is added")
			Eventually(func(g Gomega) []string {
				got := &dpuservicev1.DPUServiceNAD{}
				g.Expect(testClient.Get(ctx, client.ObjectKeyFromObject(dpuServiceNAD), got)).To(Succeed())
				return got.Finalizers
			}).WithTimeout(10 * time.Second).Should(ConsistOf([]string{dpuServiceNADFinalizer}))
		})
	})

	Context("When checking the behavior in a single DPU cluster", func() {
		var (
			testNS           *corev1.Namespace
			dpuCluster       provisioningv1.DPUCluster
			dpuClusterClient client.Client
		)
		BeforeEach(func() {
			By("Creating the namespaces")
			testNS = &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{GenerateName: "testns-"}}
			Expect(testClient.Create(ctx, testNS)).To(Succeed())
			DeferCleanup(testClient.Delete, ctx, testNS)

			By("Adding fake kamaji cluster")
			dpuCluster = testutils.GetTestDPUCluster(testNS.Name, "envtest")
			kamajiSecret, err := testutils.GetFakeKamajiClusterSecretFromEnvtest(dpuCluster, cfg)
			Expect(err).NotTo(HaveOccurred())
			Expect(testClient.Create(ctx, kamajiSecret)).To(Succeed())
			DeferCleanup(testClient.Delete, ctx, kamajiSecret)

			// Create the DPUCluster object.
			Expect(testClient.Create(ctx, &dpuCluster)).To(Succeed())
			DeferCleanup(testClient.Delete, ctx, &dpuCluster)
			patcher := patch.NewSerialPatcher(&dpuCluster, testClient)

			// mark the cluster as ready so that the remoteCache treats it as ready
			dpuCluster.Status.Phase = provisioningv1.PhaseReady
			Expect(patcher.Patch(ctx, &dpuCluster, patch.WithFieldOwner("dpuservicechain-controller"))).To(Succeed())

			// create a client for the DPUCluster
			dpuClusterClient, err = dpucluster.NewConfig(testClient, &dpuCluster).Client(ctx)
			Expect(err).ToNot(HaveOccurred())
		})

		It("should reconcile NAD in DPU cluster", func() {
			By(" Creating the DPUServiceNAD resource ")
			DPUServiceNAD := getDPUServiceNADWithSpec(testNS.Name)
			Expect(testClient.Create(ctx, DPUServiceNAD)).To(Succeed())
			DeferCleanup(testutils.CleanupAndWait, ctx, testClient, DPUServiceNAD)

			Eventually(func(g Gomega) {
				got := &unstructured.Unstructured{}
				got.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   nadGroup,
					Version: nadVersion,
					Kind:    nadKind,
				})
				g.Expect(dpuClusterClient.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: "mynad"}, got)).To(Succeed())
				g.Expect(got.GetLabels()).To(HaveKeyWithValue("dpu.nvidia.com/dpuservicenad-name", "mynad"))
				g.Expect(got.GetLabels()).To(HaveKeyWithValue("dpu.nvidia.com/dpuservicenad-namespace", testNS.Name))
				g.Expect(got.GetLabels()).To(HaveKeyWithValue("labelTest", "labelTestValue"))
				g.Expect(got.GetAnnotations()).To(HaveKeyWithValue("annotTest", "annotTestValue"))

				var data map[string]any
				spec, found, err := unstructured.NestedString(got.Object, "spec", "config")
				g.Expect(err).ToNot(HaveOccurred())
				g.Expect(found).To(BeTrue())
				err = json.Unmarshal([]byte(spec), &data)
				g.Expect(err).ToNot(HaveOccurred())
				g.Expect(data["cniVersion"].(string)).To(Equal("0.3.1"))
				g.Expect(data["type"].(string)).To(Equal("ovs"))
				g.Expect(data["bridge"].(string)).To(Equal("test-ovsbridge"))
				g.Expect(data["interface_type"].(string)).To(Equal("dpdk"))
				g.Expect(int(data["mtu"].(float64))).To(Equal(1500))
			}).WithTimeout(60 * time.Second).Should(Succeed())

			By("Removing the DPUServiceNAD resource")
			Expect(testClient.Delete(ctx, DPUServiceNAD)).To(Succeed())

			Eventually(func(g Gomega) {
				got := &unstructured.UnstructuredList{}
				got.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   nadGroup,
					Version: nadVersion,
					Kind:    "NetworkAttachmentDefinitionList",
				})
				g.Expect(dpuClusterClient.List(ctx, got)).To(Succeed())
				g.Expect(got.Items).To(BeEmpty())
			}).WithTimeout(60 * time.Second).Should(Succeed())
		})

		It("should successfully detect update on the NAD", func() {
			By("Create DPUServiceNAD")
			dpuServiceNAD := getDPUServiceNADWithSpec(testNS.Name)
			Expect(testClient.Create(ctx, dpuServiceNAD)).To(Succeed())
			DeferCleanup(testutils.CleanupAndWait, ctx, testClient, dpuServiceNAD)

			By("Verify NAD is created")
			Eventually(func(g Gomega) {
				got := &unstructured.Unstructured{}
				got.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   nadGroup,
					Version: nadVersion,
					Kind:    nadKind,
				})
				g.Expect(dpuClusterClient.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: "mynad"}, got)).To(Succeed())
			}, timeout, interval).Should(Succeed())

			By("Verify NAD")
			got := &unstructured.Unstructured{}
			got.SetGroupVersionKind(schema.GroupVersionKind{
				Group:   nadGroup,
				Version: nadVersion,
				Kind:    nadKind,
			})
			Expect(dpuClusterClient.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: "mynad"}, got)).To(Succeed())
			Expect(got.GetLabels()).To(HaveKeyWithValue("dpu.nvidia.com/dpuservicenad-name", "mynad"))
			Expect(got.GetLabels()).To(HaveKeyWithValue("dpu.nvidia.com/dpuservicenad-namespace", testNS.Name))
			Expect(got.GetLabels()).To(HaveKeyWithValue("labelTest", "labelTestValue"))
			Expect(got.GetAnnotations()).To(HaveKeyWithValue("annotTest", "annotTestValue"))

			By("Verify watcher is created")
			Eventually(func(g Gomega) {
				remoteClient, err := remoteCache.GetClient(client.ObjectKeyFromObject(&dpuCluster))
				g.Expect(err).NotTo(HaveOccurred())
				g.Expect(remoteClient).NotTo(BeNil())
			}, timeout, interval).Should(Succeed())

			// now that we know the watcher is created, we can test the nad update
			// detection logic. For that purpose we create a fresh DPUServiceNAD. This is needed
			// because the remote cache detecting a new dpuCluster and our reconciler creating a new watcher
			// for nad kind is an operation that is subject to race conditions. With the new
			// DPUServiceNAD we can reduce unwanted reconciliation loops to a minimum.
			By("Recreate DPUServiceNAD")
			Expect(testutils.CleanupAndWait(ctx, testClient, dpuServiceNAD)).To(Succeed())
			dpuServiceNAD = getDPUServiceNADWithSpec(testNS.Name)
			Expect(testClient.Create(ctx, dpuServiceNAD)).To(Succeed())
			DeferCleanup(testutils.CleanupAndWait, ctx, testClient, dpuServiceNAD)

			By("Verify NAD is created")
			got = &unstructured.Unstructured{}
			got.SetGroupVersionKind(schema.GroupVersionKind{
				Group:   nadGroup,
				Version: nadVersion,
				Kind:    nadKind,
			})
			Eventually(func(g Gomega) {
				g.Expect(dpuClusterClient.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: "mynad"}, got)).NotTo(HaveOccurred())
				data, found, err := unstructured.NestedString(got.Object, "spec", "config")
				g.Expect(err).ToNot(HaveOccurred())
				g.Expect(found).To(BeTrue())
				spec := unmarshallNadSpec(data)
				g.Expect(spec).To(HaveKeyWithValue("cniVersion", "0.3.1"))
				g.Expect(spec).To(HaveKeyWithValue("type", "ovs"))
				g.Expect(spec).To(HaveKeyWithValue("bridge", "test-ovsbridge"))
				g.Expect(spec).To(HaveKeyWithValue("mtu", float64(1500)))
				g.Expect(spec).To(HaveKeyWithValue("ipam", map[string]any{
					"type": "nv-ipam",
				}))
			}, timeout, interval).Should(Succeed())

			Eventually(func(g Gomega) {
				By("Update NAD")
				gotNad := &unstructured.Unstructured{}
				gotNad.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   nadGroup,
					Version: nadVersion,
					Kind:    nadKind,
				})
				g.Expect(dpuClusterClient.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: "mynad"}, gotNad)).NotTo(HaveOccurred())
				err := unstructured.SetNestedField(gotNad.Object, getDummyConfig(), "spec", "config")
				g.Expect(err).ToNot(HaveOccurred())
				gotNad.SetManagedFields(nil)
				gotNad.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   nadGroup,
					Version: nadVersion,
					Kind:    nadKind})
				g.Expect(dpuClusterClient.Patch(ctx, gotNad, client.Apply, client.ForceOwnership, client.FieldOwner("dpuservicenad-controller"))).To(Succeed())

				By("Verify NAD is reverted")
				g.Eventually(func(g Gomega) {
					got := &unstructured.Unstructured{}
					got.SetGroupVersionKind(schema.GroupVersionKind{
						Group:   nadGroup,
						Version: nadVersion,
						Kind:    nadKind,
					})
					g.Expect(dpuClusterClient.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: "mynad"}, got)).To(Succeed())
					data, found, err := unstructured.NestedString(got.Object, "spec", "config")
					g.Expect(err).ToNot(HaveOccurred())
					g.Expect(found).To(BeTrue())
					spec := unmarshallNadSpec(data)
					g.Expect(spec).To(HaveKeyWithValue("cniVersion", "0.3.1"))
					g.Expect(spec).To(HaveKeyWithValue("type", "ovs"))
					g.Expect(spec).To(HaveKeyWithValue("bridge", "test-ovsbridge"))
					g.Expect(spec).To(HaveKeyWithValue("mtu", float64(1500)))
					g.Expect(spec).To(HaveKeyWithValue("ipam", map[string]any{
						"type": "nv-ipam",
					}))
				}, timeout, interval).Should(Succeed())
			}, timeout, interval).MustPassRepeatedly(10).Should(Succeed())
		})

		It("should reconcile NAD with IPAM in DPU cluster", func() {
			By("Creating the DPUServiceNAD resource ")
			DPUServiceNAD := getDPUServiceNADWithSpec(testNS.Name)
			DPUServiceNAD.Spec.IPAM = true
			Expect(testClient.Create(ctx, DPUServiceNAD)).To(Succeed())

			Eventually(func(g Gomega) {
				got := &unstructured.Unstructured{}
				got.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   nadGroup,
					Version: nadVersion,
					Kind:    nadKind,
				})
				g.Expect(dpuClusterClient.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: "mynad"}, got)).To(Succeed())
				g.Expect(got.GetLabels()).To(HaveKeyWithValue("dpu.nvidia.com/dpuservicenad-name", "mynad"))
				g.Expect(got.GetLabels()).To(HaveKeyWithValue("dpu.nvidia.com/dpuservicenad-namespace", testNS.Name))
				g.Expect(got.GetLabels()).To(HaveKeyWithValue("labelTest", "labelTestValue"))
				g.Expect(got.GetAnnotations()).To(HaveKeyWithValue("annotTest", "annotTestValue"))

				var data map[string]any
				spec, found, err := unstructured.NestedString(got.Object, "spec", "config")
				g.Expect(err).ToNot(HaveOccurred())
				g.Expect(found).To(BeTrue())
				err = json.Unmarshal([]byte(spec), &data)
				g.Expect(err).ToNot(HaveOccurred())
				ipamData, ok := data["ipam"].(map[string]any)
				g.Expect(ok).To(BeTrue())
				g.Expect(ipamData["type"].(string)).To(Equal("nv-ipam"))
				g.Expect(data["cniVersion"].(string)).To(Equal("0.3.1"))
				g.Expect(data["type"].(string)).To(Equal("ovs"))
				g.Expect(data["bridge"].(string)).To(Equal("test-ovsbridge"))
				g.Expect(data["interface_type"].(string)).To(Equal("dpdk"))
				g.Expect(int(data["mtu"].(float64))).To(Equal(1500))

			}).WithTimeout(60 * time.Second).Should(Succeed())

			By("Removing the DPUServiceNAD resource")
			Expect(testClient.Delete(ctx, DPUServiceNAD)).To(Succeed())

			Eventually(func(g Gomega) {
				got := &unstructured.Unstructured{}
				got.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   nadGroup,
					Version: nadVersion,
					Kind:    nadKind,
				})
				err := dpuClusterClient.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: "mynad"}, got)
				g.Expect(apierrors.IsNotFound(err)).To(BeTrue())
			}).WithTimeout(60 * time.Second).Should(Succeed())
		})

		It("should reconcile NAD with ResourceName(vf) annotation in DPU cluster", func() {
			By("Creating the DPUServiceNAD resource ")
			DPUServiceNAD := getDPUServiceNADWithSpec(testNS.Name)
			DPUServiceNAD.Spec.ResourceType = "vf"
			Expect(testClient.Create(ctx, DPUServiceNAD)).To(Succeed())

			// refer this NAD in dpuserviceinterface and dpuservice
			Eventually(func(g Gomega) {
				got := &unstructured.Unstructured{}
				got.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   nadGroup,
					Version: nadVersion,
					Kind:    nadKind,
				})
				g.Expect(dpuClusterClient.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: "mynad"}, got)).To(Succeed())
				g.Expect(got.GetLabels()).To(HaveKeyWithValue("dpu.nvidia.com/dpuservicenad-name", "mynad"))
				g.Expect(got.GetLabels()).To(HaveKeyWithValue("dpu.nvidia.com/dpuservicenad-namespace", testNS.Name))
				g.Expect(got.GetLabels()).To(HaveKeyWithValue("labelTest", "labelTestValue"))
				g.Expect(got.GetAnnotations()).To(HaveKeyWithValue("annotTest", "annotTestValue"))
				g.Expect(got.GetAnnotations()).To(HaveKeyWithValue("k8s.v1.cni.cncf.io/resourceName", "nvidia.com/bf_vf"))

			}).WithTimeout(60 * time.Second).Should(Succeed())

			By("Removing the DPUServiceNAD resource")
			Expect(testClient.Delete(ctx, DPUServiceNAD)).To(Succeed())

			Eventually(func(g Gomega) {
				got := &unstructured.Unstructured{}
				got.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   nadGroup,
					Version: nadVersion,
					Kind:    nadKind,
				})
				err := dpuClusterClient.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: "mynad"}, got)
				g.Expect(apierrors.IsNotFound(err)).To(BeTrue())
			}).WithTimeout(60 * time.Second).Should(Succeed())
		})

		It("should reconcile NAD in DPU cluster with maximum name length", func() {
			By("Creating the DPUServiceNAD resource")
			dpuServiceNAD := getDPUServiceNADWithSpec(testNS.Name)
			dpuServiceNAD.Name = utilrand.String(63)

			Expect(testClient.Create(ctx, dpuServiceNAD)).To(Succeed())
			DeferCleanup(testutils.CleanupAndWait, ctx, testClient, dpuServiceNAD)

			Eventually(func(g Gomega) {
				got := &unstructured.Unstructured{}
				got.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   "k8s.cni.cncf.io",
					Version: "v1",
					Kind:    "NetworkAttachmentDefinition",
				})
				g.Expect(dpuClusterClient.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: dpuServiceNAD.Name}, got)).To(Succeed())
			}).WithTimeout(60 * time.Second).Should(Succeed())
		})

		It("should fail to create a DPUServiceNAD with name exceeding the maximum length", func() {
			By("Create ServiceInterfaceSet, without Node Selector")
			dpuServiceNAD := getDPUServiceNADWithSpec(testNS.Name)
			dpuServiceNAD.Name = utilrand.String(64)
			Expect(testClient.Create(ctx, dpuServiceNAD)).To(HaveOccurred())
		})

		It("should reconcile NAD with chained CNI in DPU cluster", func() {
			By("Creating the DPUServiceNAD resource with chained CNI")
			DPUServiceNAD := getDPUServiceNADWithChainedCNI(testNS.Name)

			Expect(testClient.Create(ctx, DPUServiceNAD)).To(Succeed())
			DeferCleanup(testutils.CleanupAndWait, ctx, testClient, DPUServiceNAD)

			Eventually(func(g Gomega) {
				got := &unstructured.Unstructured{}
				got.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   "k8s.cni.cncf.io",
					Version: "v1",
					Kind:    "NetworkAttachmentDefinition",
				})
				g.Expect(dpuClusterClient.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: "mynad"}, got)).To(Succeed())

				var data map[string]interface{}
				spec, found, err := unstructured.NestedString(got.Object, "spec", "config")
				g.Expect(err).ToNot(HaveOccurred())
				g.Expect(found).To(BeTrue())
				err = json.Unmarshal([]byte(spec), &data)
				g.Expect(err).ToNot(HaveOccurred())

				// Check chained CNI structure
				g.Expect(data["cniVersion"].(string)).To(Equal("0.3.1"))
				g.Expect(data["name"].(string)).To(Equal("mynad"))

				// Check plugins array exists
				plugins, ok := data["plugins"].([]interface{})
				g.Expect(ok).To(BeTrue())
				g.Expect(plugins).To(HaveLen(2))

				// Check first plugin (ovs)
				ovsPlugin, ok := plugins[0].(map[string]interface{})
				g.Expect(ok).To(BeTrue())
				g.Expect(ovsPlugin["type"].(string)).To(Equal("ovs"))
				g.Expect(ovsPlugin["bridge"].(string)).To(Equal("test-ovsbridge"))
				g.Expect(ovsPlugin["interface_type"].(string)).To(Equal("dpdk"))
				g.Expect(int(ovsPlugin["mtu"].(float64))).To(Equal(1500))

				// Check second plugin (rdma)
				rdmaPlugin, ok := plugins[1].(map[string]interface{})
				g.Expect(ok).To(BeTrue())
				g.Expect(rdmaPlugin["type"].(string)).To(Equal("rdma"))

			}).WithTimeout(60 * time.Second).Should(Succeed())

			By("Removing the DPUServiceNAD resource")
			Expect(testClient.Delete(ctx, DPUServiceNAD)).To(Succeed())

			Eventually(func(g Gomega) {
				got := &unstructured.UnstructuredList{}
				got.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   "k8s.cni.cncf.io",
					Version: "v1",
					Kind:    "NetworkAttachmentDefinitionList",
				})
				g.Expect(dpuClusterClient.List(ctx, got)).To(Succeed())
				g.Expect(got.Items).To(BeEmpty())
			}).WithTimeout(60 * time.Second).Should(Succeed())
		})
	})
	Context("When checking the behavior on multiple DPU clusters", func() {
		var (
			testNS            *corev1.Namespace
			dpuCluster1       provisioningv1.DPUCluster
			dpuCluster2       provisioningv1.DPUCluster
			dpuCluster1Client client.Client
			dpuCluster2Client client.Client
		)

		BeforeEach(func() {
			testNS = &corev1.Namespace{
				ObjectMeta: metav1.ObjectMeta{
					GenerateName: "testns-",
				},
			}
			Expect(testClient.Create(ctx, testNS)).To(Succeed())
			DeferCleanup(testClient.Delete, ctx, testNS)

			By("Adding fake kamaji cluster 1 with label dpucluster=cluster1 using testEnv1")
			dpuCluster1 = testutils.GetTestDPUCluster(testNS.Name, "cluster1")
			dpuCluster1.Labels = map[string]string{"dpucluster": "cluster1"}
			kamajiSecret1, err := testutils.GetFakeKamajiClusterSecretFromEnvtest(dpuCluster1, cfg1)
			Expect(err).NotTo(HaveOccurred())
			Expect(testClient.Create(ctx, kamajiSecret1)).To(Succeed())
			DeferCleanup(testutils.CleanupAndWait, ctx, testClient, kamajiSecret1)

			Expect(testClient.Create(ctx, &dpuCluster1)).To(Succeed())
			DeferCleanup(testutils.CleanupAndWait, ctx, testClient, &dpuCluster1)
			patcher1 := patch.NewSerialPatcher(&dpuCluster1, testClient)

			// mark the cluster as ready so that the remoteCache treats it as ready
			dpuCluster1.Status.Phase = provisioningv1.PhaseReady
			Expect(patcher1.Patch(ctx, &dpuCluster1, patch.WithFieldOwner("test"))).To(Succeed())

			dpuCluster1Client, err = dpucluster.NewConfig(testClient, &dpuCluster1).Client(ctx)
			Expect(err).ToNot(HaveOccurred())

			By("Adding fake kamaji cluster 2 with label dpucluster=cluster2 using testEnv2")
			dpuCluster2 = testutils.GetTestDPUCluster(testNS.Name, "cluster2")
			dpuCluster2.Labels = map[string]string{"dpucluster": "cluster2"}
			kamajiSecret2, err := testutils.GetFakeKamajiClusterSecretFromEnvtest(dpuCluster2, cfg2)
			Expect(err).NotTo(HaveOccurred())
			Expect(testClient.Create(ctx, kamajiSecret2)).To(Succeed())
			DeferCleanup(testutils.CleanupAndWait, ctx, testClient, kamajiSecret2)

			Expect(testClient.Create(ctx, &dpuCluster2)).To(Succeed())
			DeferCleanup(testutils.CleanupAndWait, ctx, testClient, &dpuCluster2)
			patcher2 := patch.NewSerialPatcher(&dpuCluster2, testClient)

			// mark the cluster as ready so that the remoteCache treats it as ready
			dpuCluster2.Status.Phase = provisioningv1.PhaseReady
			Expect(patcher2.Patch(ctx, &dpuCluster2, patch.WithFieldOwner("test"))).To(Succeed())

			dpuCluster2Client, err = dpucluster.NewConfig(testClient, &dpuCluster2).Client(ctx)
			Expect(err).ToNot(HaveOccurred())
		})
		It("should reconcile resources in all DPU Clusters when DPUClusterSelector is not set", func() {
			By("Creating DPUServiceNAD without DPUClusterSelector")
			dpuServiceNAD := getMinimalDPUServiceNAD(testNS.Name)
			dpuServiceNAD.Name = "no-selector-nad"
			Expect(testClient.Create(ctx, dpuServiceNAD)).To(Succeed())
			DeferCleanup(testutils.CleanupAndWait, ctx, testClient, dpuServiceNAD)

			By("Verifying NetworkAttachmentDefinition is created in cluster1")
			Eventually(func(g Gomega) {
				got := &unstructured.Unstructured{}
				got.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   "k8s.cni.cncf.io",
					Version: "v1",
					Kind:    "NetworkAttachmentDefinition",
				})
				g.Expect(dpuCluster1Client.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: "no-selector-nad"}, got)).To(Succeed())
			}).WithTimeout(10 * time.Second).Should(Succeed())

			By("Verifying NetworkAttachmentDefinition is created in cluster2")
			Eventually(func(g Gomega) {
				got := &unstructured.Unstructured{}
				got.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   "k8s.cni.cncf.io",
					Version: "v1",
					Kind:    "NetworkAttachmentDefinition",
				})
				g.Expect(dpuCluster2Client.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: "no-selector-nad"}, got)).To(Succeed())
			}).WithTimeout(10 * time.Second).Should(Succeed())
		})

		It("should reconcile resources in all matching DPU Clusters", func() {
			By("Creating DPUServiceNAD with DPUClusterSelector matching dpucluster=cluster1")
			dpuServiceNAD := getMinimalDPUServiceNAD(testNS.Name)
			dpuServiceNAD.Name = "multi-cluster-nad"
			dpuServiceNAD.Spec.DPUClusterSelector = &metav1.LabelSelector{
				MatchLabels: map[string]string{"dpucluster": "cluster1"},
			}
			Expect(testClient.Create(ctx, dpuServiceNAD)).To(Succeed())
			DeferCleanup(testutils.CleanupAndWait, ctx, testClient, dpuServiceNAD)

			By("Verifying NetworkAttachmentDefinition is created in cluster1")
			Eventually(func(g Gomega) {
				got := &unstructured.Unstructured{}
				got.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   "k8s.cni.cncf.io",
					Version: "v1",
					Kind:    "NetworkAttachmentDefinition",
				})
				g.Expect(dpuCluster1Client.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: "multi-cluster-nad"}, got)).To(Succeed())
			}).WithTimeout(10 * time.Second).Should(Succeed())

			By("Verifying NetworkAttachmentDefinition is NOT created in cluster2")
			Consistently(func(g Gomega) {
				got := &unstructured.Unstructured{}
				got.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   "k8s.cni.cncf.io",
					Version: "v1",
					Kind:    "NetworkAttachmentDefinition",
				})
				err := dpuCluster2Client.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: "multi-cluster-nad"}, got)
				g.Expect(apierrors.IsNotFound(err)).To(BeTrue())
			}).WithTimeout(5 * time.Second).Should(Succeed())
		})

		It("should remove resources from non matching DPU Clusters", func() {
			By("Creating DPUServiceNAD without DPUClusterSelector (matches all clusters)")
			dpuServiceNAD := getMinimalDPUServiceNAD(testNS.Name)
			dpuServiceNAD.Name = "all-clusters-nad"
			Expect(testClient.Create(ctx, dpuServiceNAD)).To(Succeed())
			DeferCleanup(testutils.CleanupAndWait, ctx, testClient, dpuServiceNAD)

			By("Verifying NetworkAttachmentDefinition is created in both clusters")
			Eventually(func(g Gomega) {
				got1 := &unstructured.Unstructured{}
				got1.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   "k8s.cni.cncf.io",
					Version: "v1",
					Kind:    "NetworkAttachmentDefinition",
				})
				g.Expect(dpuCluster1Client.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: "all-clusters-nad"}, got1)).To(Succeed())
				got2 := &unstructured.Unstructured{}
				got2.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   "k8s.cni.cncf.io",
					Version: "v1",
					Kind:    "NetworkAttachmentDefinition",
				})
				g.Expect(dpuCluster2Client.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: "all-clusters-nad"}, got2)).To(Succeed())
			}).WithTimeout(10 * time.Second).Should(Succeed())

			By("Updating DPUServiceNAD to only match dpucluster=cluster1")
			Eventually(func(g Gomega) {
				g.Expect(testClient.Get(ctx, client.ObjectKeyFromObject(dpuServiceNAD), dpuServiceNAD)).To(Succeed())
				dpuServiceNAD.Spec.DPUClusterSelector = &metav1.LabelSelector{
					MatchLabels: map[string]string{"dpucluster": "cluster1"},
				}
				dpuServiceNAD.SetManagedFields(nil)
				dpuServiceNAD.SetGroupVersionKind(dpuservicev1.DPUServiceNADGroupVersionKind)
				g.Expect(testClient.Patch(ctx, dpuServiceNAD, client.Apply, client.FieldOwner("test"))).To(Succeed())
			}).WithTimeout(10 * time.Second).Should(Succeed())

			By("Verifying NetworkAttachmentDefinition is still in cluster1")
			Eventually(func(g Gomega) {
				got := &unstructured.Unstructured{}
				got.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   "k8s.cni.cncf.io",
					Version: "v1",
					Kind:    "NetworkAttachmentDefinition",
				})
				g.Expect(dpuCluster1Client.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: "all-clusters-nad"}, got)).To(Succeed())
			}).WithTimeout(10 * time.Second).Should(Succeed())

			By("Verifying NetworkAttachmentDefinition is removed from cluster2")
			Eventually(func(g Gomega) {
				got := &unstructured.Unstructured{}
				got.SetGroupVersionKind(schema.GroupVersionKind{
					Group:   "k8s.cni.cncf.io",
					Version: "v1",
					Kind:    "NetworkAttachmentDefinition",
				})
				err := dpuCluster2Client.Get(ctx, client.ObjectKey{Namespace: testNS.Name, Name: "all-clusters-nad"}, got)
				g.Expect(apierrors.IsNotFound(err)).To(BeTrue())
			}).WithTimeout(10 * time.Second).Should(Succeed())
		})
	})
})

func getMinimalDPUServiceNAD(namespace string) *dpuservicev1.DPUServiceNAD {
	return &dpuservicev1.DPUServiceNAD{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "dpuservicenad",
			Namespace: namespace,
		},
		Spec: dpuservicev1.DPUServiceNADSpec{
			ResourceType: "sf",
		},
	}
}

func getDummyConfig() string {
	config := map[string]any{
		"cniVersion": "0.3.1",
		"type":       "ovs",
		"mtu":        float64(1500),
		"bridge":     "dummy-ovsbridge",
		"ipam": map[string]any{
			"type": "nv-ipam",
		},
	}

	jsonConfig, err := json.MarshalIndent(config, "", "  ")
	Expect(err).ToNot(HaveOccurred())

	return string(jsonConfig)
}

func unmarshallNadSpec(spec string) map[string]any {
	var data map[string]any
	err := json.Unmarshal([]byte(spec), &data)
	Expect(err).ToNot(HaveOccurred())
	return data
}

func getDPUServiceNADWithSpec(namespace string) *dpuservicev1.DPUServiceNAD {
	return &dpuservicev1.DPUServiceNAD{
		ObjectMeta: metav1.ObjectMeta{
			Name:        "mynad",
			Namespace:   namespace,
			Labels:      map[string]string{"labelTest": "labelTestValue"},
			Annotations: map[string]string{"annotTest": "annotTestValue"},
		},
		Spec: dpuservicev1.DPUServiceNADSpec{
			ResourceType: "sf",
			Bridge:       "test-ovsbridge",
			ServiceMTU:   1500,
			IPAM:         true,
		},
	}
}

func getDPUServiceNADWithChainedCNI(namespace string) *dpuservicev1.DPUServiceNAD {
	rdmaType := "rdma"

	return &dpuservicev1.DPUServiceNAD{
		ObjectMeta: metav1.ObjectMeta{
			Name:        "mynad",
			Namespace:   namespace,
			Labels:      map[string]string{"labelTest": "labelTestValue"},
			Annotations: map[string]string{"annotTest": "annotTestValue"},
		},
		Spec: dpuservicev1.DPUServiceNADSpec{
			ResourceType: "sf",
			Bridge:       "test-ovsbridge",
			ServiceMTU:   1500,
			IPAM:         false,
			ChainedCNIs: []dpuservicev1.CNIPlugin{
				{
					Type: &rdmaType,
				},
			},
		},
	}
}
