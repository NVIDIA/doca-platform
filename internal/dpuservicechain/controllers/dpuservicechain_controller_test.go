...
var _ = Describe("DPUServiceChain Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			...
			// create the DPFOperatorConfig for MTU validation
			dpfOperatorConfig := getMinimalDPFOperatorConfig()
			dpfOperatorConfig.Namespace = testNS
		...
		It("should successfully reconcile a DPUServiceChain with maximum name length", func() {
			By("Create DPUServiceChain")
			dsc := getMinimalDPUServiceChain(utilrand.String(63), testNS, &metav1.LabelSelector{})
			Expect(testClient.Create(ctx, dsc)).NotTo(HaveOccurred())
			cleanupObjects = append(cleanupObjects, dsc)
			By("Verify ServiceChainSet is created")
			Eventually(func(g Gomega) {
				scs := &dpuservicev1.ServiceChainSet{ObjectMeta: metav1.ObjectMeta{Name: dsc.Name, Namespace: testNS}}
				g.Expect(dpuClusterClient.Get(ctx, client.ObjectKeyFromObject(scs), scs)).NotTo(HaveOccurred())
				cleanupObjects = append(cleanupObjects, scs)
			}, timeout*30, interval).Should(Succeed())
		})
		It("should fail to create a DPUServiceChain with name exceeding the maximum length", func() {
			By("Create DPUServiceChain")
			dsc := getMinimalDPUServiceChain(utilrand.String(64), testNS, &metav1.LabelSelector{})
		...
		It("should successfully reconcile the DPUServiceChain with valid specified MTU", func() {
			By("Create DPUServiceChain")
			dpuServiceChain := getMinimalDPUServiceChain(dscResourceName, testNS, &metav1.LabelSelector{})
			...
			cleanupObjects = append(cleanupObjects, dpuServiceChain)
			By("Verify ServiceChainSet is created")
			Eventually(func(g Gomega) {
				scs := &dpuservicev1.ServiceChainSet{ObjectMeta: metav1.ObjectMeta{Name: dscResourceName, Namespace: testNS}}
				g.Expect(dpuClusterClient.Get(ctx, client.ObjectKeyFromObject(scs), scs)).NotTo(HaveOccurred())
		...
		It("should successfully detect update on the ServiceChainSet", func() {
			By("Create DPUServiceChain")
			dpuServiceChain := createDPUServiceChain(ctx, dscResourceName, testNS, &metav1.LabelSelector{})
			cleanupObjects = append(cleanupObjects, dpuServiceChain)
			By("Verify ServiceChainSet is created")
			Eventually(func(g Gomega) {
				scs := &dpuservicev1.ServiceChainSet{ObjectMeta: metav1.ObjectMeta{Name: dscResourceName, Namespace: testNS}}
			...
			// for serviceChainSet kind is an operation that is subject to race conditions. We the new
