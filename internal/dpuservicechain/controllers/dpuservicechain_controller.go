...
func (r *DPUServiceChainReconciler) reconcile(ctx context.Context, dpuServiceChain *dpuservicev1.DPUServiceChain) (ctrl.Result, error) {
	...
	err := r.List(ctx, dpuClusters)
	if err != nil {
		return ctrl.Result{}, fmt.Errorf("error while listing DPU clusters: %w", err)
	}

	var errs []error
	for _, dpuCluster := range dpuClusters.Items {
		// This is a no-op if the watch is already registered
		if err := r.watchServiceChainSet(ctx, client.ObjectKey{Namespace: dpuCluster.Namespace, Name: dpuCluster.Name}); err != nil {
			// if err happens, most likely the connection is unhealthy
...
func (r *DPUServiceChainReconciler) watchServiceChainSet(ctx context.Context, dpuCluster client.ObjectKey) error {
...
func validateMTUValueInSwitches(r client.Client, ctx context.Context, switches []dpuservicev1.Switch) error {
	...
	for _, s := range switches {
		if *s.ServiceMTU > *highSpeedMTU {
			errs = append(errs, fmt.Errorf("MTU %d defined in switch %v can't be bigger than the high speed MTU %d", *s.ServiceMTU, s, highSpeedMTU))
		}
