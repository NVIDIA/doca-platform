...
func (r *DPUServiceChainReconciler) reconcile(ctx context.Context, dpuServiceChain *dpuservicev1.DPUServiceChain) (ctrl.Result, error) {
	...
	// start watching the ServiceChainSet in the DPU clusters
...
func (r *DPUServiceChainReconciler) createOrUpdateObjectsInDPUCluster(ctx context.Context, k8sClient client.Client, _ types.NamespacedName, dpuObject dpuservicev1.DPUServiceObject) error {
	...
	// TODO NOAM: Remove this line once the podrestart controller is implemented
	if err := validateMTUValueInSwitches(r.Client, ctx, scs.Spec.Template.Spec.Switches); err != nil {
		return err
	}
...
func (r *DPUServiceChainReconciler) SetupWithManager(mgr ctrl.Manager) error {
	c, err := ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&provisioningv1.DPUCluster{}, handler.EnqueueRequestsFromMapFunc(r.DPUClusterToDPUServiceChain)).
		Build(r)
...
// registerKindToWatcher registers the ServiceChainSet kind to the remote cache watcher
// so that the DPUServiceChain controller can watch for changes in the ServiceChainSet objects
// in the DPU clusters. This is used to trigger reconciliation of the DPUServiceChain
// when a ServiceChainSet is created, updated, or deleted in any of the DPU clusters.
func (r *DPUServiceChainReconciler) registerKindToWatcher(ctx context.Context, dpuCluster client.ObjectKey) error {
	return r.RemoteCache.Watch(ctx, dpuCluster, dpucluster.NewWatcher(dpucluster.WatcherOptions{
		Name:         "dpuservicechain-watch-servicechainset",
		Watcher:      r.controller,
		Kind:         &dpuservicev1.ServiceChainSet{},
		EventHandler: handler.EnqueueRequestsFromMapFunc(r.serviceChainSetToDPUServiceChain),
		Predicates:   []predicate.TypedPredicate[client.Object]{predicates.TypedResourceIsChanged[client.Object]()},
	}))
}

func (r *DPUServiceChainReconciler) serviceChainSetToDPUServiceChain(ctx context.Context, o client.Object) []reconcile.Request {
	log := ctrllog.FromContext(ctx)
	set, ok := o.(*dpuservicev1.ServiceChainSet)
	if !ok {
		log.Error(fmt.Errorf("expected a ServiceChainSet but got a %T", o), "Failed to convert object")
		return nil
	}
...
// validateMTUValueInSwitches ensures MTU values defined in the switches are valid. Returns error if any of them is
// invalid.
func validateMTUValueInSwitches(r client.Client, ctx context.Context, switches []dpuservicev1.Switch) error {
	if len(switches) == 0 {
		return nil
	}

	highSpeedMTU, err := getHighSpeedMTU(r, ctx)
	if err != nil {
		return err
	}
	...
	for _, s := range switches {
		if *s.ServiceMTU > *highSpeedMTU {
			errs = append(errs, fmt.Errorf("MTU %d can't be bigger than the high speed MTU %d", *s.ServiceMTU, *highSpeedMTU))
		}
...
// getHighSpeedMTU gets the high speed MTU from the DPFOperatorConfig
func getHighSpeedMTU(r client.Client, ctx context.Context) (*int, error) {
	dpfOperatorConfig, err := dpfutils.GetDPFOperatorConfig(ctx, r)
	if err != nil {
		return nil, err
	}

	return dpfOperatorConfig.Spec.Networking.HighSpeedMTU, nil
}
