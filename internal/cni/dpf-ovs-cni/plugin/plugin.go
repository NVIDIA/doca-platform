...
// SFC CNI plugin: uses Linux network namespaces (containernetworking/plugins/pkg/ns)
// and netlink, so it only builds on Linux.
...
func CmdAdd(args *skel.CmdArgs) error {
	...
	mtu := netconf.MTU
	if netconf.Args != nil && netconf.Args.CNI != nil && netconf.Args.CNI.MTU != 0 {
		envMTU := netconf.Args.CNI.MTU
		if envMTU > 528 && envMTU < 9216 {
			mtu = envMTU
		}
	}
