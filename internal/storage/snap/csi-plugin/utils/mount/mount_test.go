...
var _ = Describe("Mount Utils", func() {
	...
	Context("EnsureDirExist", func() {
		It("Exist", func() {
			Expect(os.MkdirAll(filepath.Join(tmpDir, "/stage/test-dir"), 0755)).NotTo(HaveOccurred())
			Expect(mountUtils.EnsureDirExist(filepath.Join(tmpDir, "/stage/test-dir"), 0755)).NotTo(HaveOccurred())
			stat, err := os.Stat(filepath.Join(tmpDir, "/stage/test-dir"))
			Expect(err).NotTo(HaveOccurred())
