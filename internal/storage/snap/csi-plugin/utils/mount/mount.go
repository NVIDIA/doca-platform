...
type Utils interface {
	...
	// EnsureDirExist creates a directory with specified path and all parent directories if required
	EnsureDirExist(dirPath string, mode os.FileMode) error
}
...
func (m *mountUtils) EnsureFileExist(path string, mode os.FileMode) error {
	...
	if err != nil {
		return fmt.Errorf("failed to check stats for the file: %w", err)
	}
	if exist {
		return nil
	}
...
// EnsureDirExist creates a directory with specified path and all parent directories if required
func (m *mountUtils) EnsureDirExist(dirPath string, mode os.FileMode) error {
...
func (m *mountUtils) CheckMountExists(src, mountPoint string) (bool, kmount.MountInfo, error) {
	...
	if err != nil {
		return false, kmount.MountInfo{}, fmt.Errorf("failed to read mounts for src: %w", err)
	}
...
func (m *mountUtils) getMounts(src string) ([]kmount.MountInfo, error) {
	...
	for _, mnt := range mounts {
		...
		if err != nil && !kmount.IsCorruptedMnt(err) {
			return nil, fmt.Errorf("failed to check mount: %w", err)
		}
...
func (m *mountUtils) isMountPoint(mountPoint string) (bool, error) {
	...
	mounts, err := kmount.ParseMountInfo(procMountInfoPath)
	if err != nil {
		return false, fmt.Errorf("failed to read mounts: %w", err)
	}
	for _, mnt := range mounts {
		if mnt.MountPoint == mountPoint {
			return true, nil
