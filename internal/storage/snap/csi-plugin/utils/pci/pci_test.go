...
var _ = Describe("Pci Utils package test", func() {
	...
	Describe("utils", func() {
		...
		Context("LoadDriver", func() {
			...
			It("wrong driver", func() {
				...
				Expect(pciUtils.LoadDriver("0000:b1:00.2", "nvme")).To(
					MatchError(ContainSubstring("device is already bound to a different driver")))
			})
		})
		...
		Context("IsSRIOVEnabled", func() {
			It("enabled - total VFs > 0", func() {
				fakefs.GinkgoConfigureFakeFS(&fsRoot, fakefs.Config{
					Dirs: []fakefs.DirEntry{
						{Path: "/sys/devices/pci0000:b0/0000:b0:04.0/0000:b1:00.2"},
						{Path: "/sys/bus/pci/devices"},
				...
				enabled, err := pciUtils.IsSRIOVEnabled("0000:b1:00.2")
				Expect(err).NotTo(HaveOccurred())
				Expect(enabled).To(BeTrue())
			})
			It("disabled - total VFs = 0", func() {
				fakefs.GinkgoConfigureFakeFS(&fsRoot, fakefs.Config{
					Dirs: []fakefs.DirEntry{
						{Path: "/sys/devices/pci0000:b0/0000:b0:04.0/0000:b1:00.2"},
						{Path: "/sys/bus/pci/devices"},
			...
			It("disabled - file does not exist", func() {
				fakefs.GinkgoConfigureFakeFS(&fsRoot, fakefs.Config{
					Dirs: []fakefs.DirEntry{
						{Path: "/sys/devices/pci0000:b0/0000:b0:04.0/0000:b1:00.2"},
						{Path: "/sys/bus/pci/devices"},
			...
			It("failed", func() {
				...
				_, err := pciUtils.IsSRIOVEnabled("0000:b1:00.2")
