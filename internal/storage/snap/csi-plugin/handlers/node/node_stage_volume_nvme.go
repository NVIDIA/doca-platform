...
// stageNVMe implements stage volume logic for NVMe emulation mode
func (h *node) stageNVMe(ctx context.Context, stagingPath string, pciAddress string, nvmeNsID int32) (*csi.NodeStageVolumeResponse, error) {
	reqLog := logr.FromContextOrDiscard(ctx)
	reqLog.Info("staging volume", "stagingPath", stagingPath, "pciAddress", pciAddress, "nvmeNsID", nvmeNsID)
	blockDevName, err := h.getBlockDevice(ctx, pciAddress, nvmeNsID)
	if err != nil {
		return nil, err
	}
	devicePath := filepath.Join("/dev", blockDevName)
	if err := h.mount.EnsureFileExist(stagingPath, 0644); err != nil {
	...
	exist, _, err := h.mount.CheckMountExists(devicePath, stagingPath)
	if err != nil {
		reqLog.Error(err, "error occurred while checking if the volume is staged")
		return nil, status.Error(codes.Internal, "error occurred while checking if the volume is staged")
	}
	if exist {
		reqLog.Info("volume already staged")
		return &csi.NodeStageVolumeResponse{}, nil
	}
	if err := h.mount.Mount(devicePath, stagingPath, "", []string{"bind"}); err != nil {
		reqLog.Error(err, "failed to stage volume, bind mount failed")
		return nil, status.Error(codes.Internal, "failed to stage volume, bind mount failed")
	}
	return &csi.NodeStageVolumeResponse{}, nil
}
...
func (h *node) getBlockDevice(ctx context.Context, pciAddress string, nvmeNsID int32) (string, error) {
	...
	if !getBlockDeviceNameForNSErrorIsRetryable(err) {
...
// returns true if error returned by GetBlockDeviceNameForNS is retryable
func getBlockDeviceNameForNSErrorIsRetryable(err error) bool {
	return errors.Is(err, utilsNvme.ErrBlockDeviceNotFound) || errors.Is(err, utilsNvme.ErrBlockDeviceIsInvalid)
}
