...
func New(pluginConfig config.PluginConfig, options ...Option) (*ServiceManager, error) {
	...
	if opts.identityHandler == nil {
		opts.identityHandler, err = identity.New(pluginConfig.Common)
		if err != nil {
	...
	case config.PluginModeController:
		...
		if opts.clusterhelper == nil {
			ctrlConfig, err := ctrl.GetConfig()
			if err != nil {
				return nil, fmt.Errorf("failed to get config for host cluster: %v", err)
			}
			opts.clusterhelper = clusterhelper.New(ctrlConfig, pluginConfig.Controller)
		}
		...
		if opts.controllerHandler == nil {
			opts.controllerHandler = controller.New(pluginConfig.Common, pluginConfig.Controller, opts.clusterhelper)
		}
	...
	case config.PluginModeNode:
		...
		if opts.preconfigure == nil {
			opts.preconfigure = preconfigure.New(pluginConfig.Common, pluginConfig.Node, nodeRuntimeConfig, pciUtils)
		}
		mgr.runner.AddService("preconfigure", opts.preconfigure)

		klog.V(2).Info("register node service")
		if opts.nodeHandler == nil {
			opts.nodeHandler = node.New(pluginConfig.Common, pluginConfig.Node, nodeRuntimeConfig,
