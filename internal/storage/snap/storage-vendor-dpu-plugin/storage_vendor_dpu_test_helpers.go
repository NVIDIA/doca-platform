...
func setupTestServer(mockClient *mockRPCClient) (*StoragePluginServer, func()) {
	// Save the original NewRPCClient function
	origNewRPCClient := NewRPCClient
	// Override NewRPCClient to return our mock
	NewRPCClient = func(socketPath string) (*rpcClient, error) {
		return &rpcClient{
			conn:  nil, // We don't need a real connection for tests
			rpcID: 1,
		}, nil
	}
