...
func TestCreateGRPCServer(t *testing.T) {
	// Create a temporary socket file path for testing
	testSocketPath := "/tmp/test_storage_vendor_dpu.sock"
	defer func() {
		if err := os.Remove(testSocketPath); err != nil {
			t.Logf("Warning: Failed to remove test socket file: %v", err)
		}
	}()

	server, listener, err := CreateGRPCServer(testSocketPath)
...
func TestGetPluginInfo(t *testing.T) {
	...
	require.Equal(t, fmt.Sprintf("storage.dpu.%s.com", pluginName), resp.Name)
...
func TestGetSNAPProvider(t *testing.T) {
	...
	require.Equal(t, pluginName, resp.ProviderName)
}
...
func TestCreateDevice(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			server, cleanup := setupTestServer(tt.mockClient)
...
func TestConfigurablePaths(t *testing.T) {
	...
	require.Equal(t, defaultPluginRPCSocketPath, GetPluginRPCSocketPath())
	require.Equal(t, defaultSNAPRPCSocketPath, GetSNAPRPCSocketPath())
	require.Equal(t, pluginName, GetPluginName())
	...
	server := &StoragePluginServer{}
	err := server.getRPCClient()
	// We expect an error since the custom path doesn't exist, but we can still check the path was used
	require.Error(t, err)
	require.Contains(t, err.Error(), customSNAPPath)
}
