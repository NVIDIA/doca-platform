...
type RPCClient interface {
	...
	// FsdevAioCreate creates an AIO filesystem device
	FsdevAioCreate(name string, volumePath string) error
	// FsdevAioDelete deletes an AIO filesystem device
	FsdevAioDelete(name string) error
...
// FsdevAioCreate creates an AIO filesystem with the given parameters
func (c *rpcClient) FsdevAioCreate(name string, volumePath string) error {
	if name == "" || volumePath == "" {
		return fmt.Errorf("name and volumePath are required")
	}

	createRequest := map[string]interface{}{
		"name":      name,
		"root_path": volumePath,
	}

	klog.Infof("Creating AIO filesystem with request: %+v", createRequest)
	_, err := c.Call("fsdev_aio_create", createRequest)
	if err != nil {
		return fmt.Errorf("failed to create AIO filesystem: %w", err)
	}
	klog.Infof("Successfully created AIO filesystem: %s", name)
	return nil
}

// FsdevAioDelete deletes an AIO filesystem
func (c *rpcClient) FsdevAioDelete(name string) error {
	params := map[string]interface{}{
		"name": name,
	}

	klog.Infof("Deleting AIO filesystem: %+v", name)
	_, err := c.Call("fsdev_aio_delete", params)
	if err != nil {
		return fmt.Errorf("failed to delete AIO filesystem: %w", err)
	}
	klog.Infof("Successfully deleted AIO filesystem: %s", name)
	return nil
}
