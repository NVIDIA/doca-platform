...
func Test_TreePrefix(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			g.Expect(output.String()).Should(MatchTable(tt.expectPrefix), "output:\n%s\n", output.String())
		})
	}
}
...
type Table struct {
	...
	missmatchLine       string
	expectedLine        string
	missmatchLineNumber int
}
...
func (t *Table) Match(actual interface{}) (bool, error) {
	...
	if len(t.tableData) > len(tableParts) {
		return false, fmt.Errorf("cannot match table, expected at least %d lines, got %d", len(t.tableData), len(tableParts))
	}

	for i := range t.tableData {
		if !strings.HasPrefix(tableParts[i], t.tableData[i]) {
			t.missmatchLine = tableParts[i]
			t.missmatchLineNumber = i
			t.expectedLine = t.tableData[i]
			return false, nil
		}
	}
	return true, nil
...
func (t *Table) FailureMessage(actual interface{}) string {
	...
	return fmt.Sprintf("Expected: %v\nReceived: %v\nMissmatch line number: %d\nExpected line: %q\nMissmatch line: %q",
		t.tableData, actualTable, t.missmatchLineNumber, t.expectedLine, t.missmatchLine)
}
