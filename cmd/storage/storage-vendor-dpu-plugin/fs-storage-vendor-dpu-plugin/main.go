...
func main() {
	...
	go func() {
		log.Println("Filesystem gRPC server starting to serve.")
		if serveErr := server.Serve(listener); serveErr != nil {
			log.Printf("Filesystem gRPC server encountered error: %v\n", serveErr)
		}
	}()

	log.Println("Filesystem gRPC server is running. Press Ctrl+C to stop.")
	...
	log.Println("Received shutdown signal. Stopping Filesystem gRPC server gracefully...")
	...
	log.Println("Filesystem gRPC server stopped.")
}
