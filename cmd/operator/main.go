...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		// so old and new pods briefly overlap on upgrade and must contend the same Lease.
		// If this ID ever changes, switch the chart back to strategy.type: Recreate
		// temporarily for that upgrade to avoid multiple active leaders.
		LeaderElectionID: "dpf-operator.dpu.nvidia.com",
