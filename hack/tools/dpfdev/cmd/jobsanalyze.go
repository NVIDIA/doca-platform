...
var jobsAnalyzeCmd = &cobra.Command{
  ...
  dpfdev jobs analyze --start 2024-01-01 --end 2024-01-31`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// Ensure branch is never empty
		if branch == "" {
			return fmt.Errorf("branch parameter cannot be empty, please specify a branch name")
		}
		...
		if branch != "" || jobName != "" || !end.IsZero() {
			filteredJobs = filterJobs(filteredJobs, branch, jobName, start, end)
		}
...
// filterJobs filters a list of jobs by branch, job name, and date range
func filterJobs(jobs []gitlab.Job, branch, jobName string, start, end time.Time) []gitlab.Job {
	if branch == "" && jobName == "" && start.IsZero() && end.IsZero() {
		return jobs
	}
